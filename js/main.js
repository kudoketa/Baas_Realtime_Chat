...
// ログアウトした直後に呼ばれる
function onLogout() {
  firebase.database().ref("users").off("value");  //ref→reference, value==user情報
...
function loadChatView() {
  ...
  favoritesRef.on("child_removed", function(favSnapshot) {
    ...
    if (dbdata.favrietes) { 
      dbdata.favorites.arrayRemove(messageId); 
    }
    ...
    $("#message-id-" + messageId).find(".message__favorite-link").find(".glyphicon").removeClass("glyphicon-star");
    $("#message-id-" + messageId).find(".message__favorite-link").find(".glyphicon").addClass("glyphicon-star-empty");
  });
  ...
  favoritesRef.on("child_added", function(favSnapshot) {
    ...
    $("#message-id-" + messageId).find(".message__favorite-link").find(".glyphicon").removeClass("glyphicon-star-empty"); 
    $("#message-id-" + messageId).find(".message__favorite-link").find(".glyphicon").addClass("glyphicon-star");
  });
}
...
function showCurrentRoom() {
  if (currentRoomName) {
    ...
    if (location.hash) { // URLの#以降がある��合はそのルームを表示
...
function createMessageDiv(messageId, message) {
  ...
  // TODO: お気に入りONのと���、���気���入り���ンクの glyphicon を設定する（glyphicon-star）
  if(dbdata.favorites && dbdata.favorites[messageId]) {
    mfl.find(".message__favorite").removeClass("glyphicon-star-empty"); 
    mfl.find(".message__favorite").addClass("glyphicon-star");
  }
...
// favoriteの表示用のdiv（jQueryオブジェクト）���作���て返す
...
$(document).ready(function() {
  ...
  $("#settings-profile-image").change(function() {
    if (this.files.length === 0) { // ファイルが選択���れていない場合
      return;
    }
    ...
    firebase.storage().ref("profile-images/" + currentUID).put(file, metadata).then(function(snapshot) {
      ...
      console.error("プロフィー��画像のアップロードに失敗:", error);
    });
  });

  // ユーザ情報設��フォームが送信されて���ページ遷移しない
